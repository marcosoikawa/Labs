...
resource "azurerm_public_ip" "AzFw-PROD-pip" {
  name                = "AzFw-PROD"
  location            = azurerm_resource_group.Hiro-AzFw.location
  resource_group_name = azurerm_resource_group.Hiro-AzFw.name
...
resource "azurerm_firewall" "AzFw-PROD" {
  ...
  ip_configuration {
    name                 = "configuration"
    subnet_id            = azurerm_virtual_network.VNET-SEC-PROD.subnet.*.id[0]
...
resource "azurerm_firewall" "AzFw-HML" {
  name                = "AzFw-HML"
  location            = azurerm_resource_group.Hiro-AzFw.location
  resource_group_name = azurerm_resource_group.Hiro-AzFw.name
  sku_name            = "AZFW_VNet"
  ...
  ip_configuration {
    name                 = "configuration"
    subnet_id            = azurerm_virtual_network.VNET-SEC-HML.subnet.*.id[0]
    public_ip_address_id = azurerm_public_ip.AzFw-HML-pip.id
  }
}
